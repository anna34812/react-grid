...
describe('DataGrid', () => {
  ...
  it('appends server pages in infinite mode when the sentinel intersects', async () => {
    ...
    try {
      render(<DataGrid columns={columns} paginationMode="infinite" />);
  ...
  it('marks movable headers draggable when enableColumnReorder is true', async () => {
    ...
    const draggableHeaders = container.querySelectorAll('.data-grid-header-row [role="columnheader"][draggable="true"]');
    expect(draggableHeaders.length).toBe(reorderColumns.length);
  });

  it('only draggable headers are columns with movable: true', async () => {
    const movableColumns = [{ field: 'id', label: 'ID', editable: false, filterable: false, movable: true }, ...columns.slice(1)];
    const { container } = render(<DataGrid columns={movableColumns} enableColumnReorder />);
    await screen.findByText('User 1');
